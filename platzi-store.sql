...
alter table CARRITO add constraint FK_MPAGO_CARRITO foreign key (MPAGO_ID)
    references MPAGO (MPAGO_ID) ondelete restrict on update restrict;

alter table CARRITO_ITEM add constraint FK_CARRITO_ITEM foreign key (ITEM_ID)
    references ITEM (ITEM_ID) ondelete restrict on update restrict;

alter table CARRITO_ITEM add constraint FK_CARRITO_ITEM2 foreign key (CARRITO_ID)
    references CARRITO (CARRITO_ID) ondelete restrict on update restrict;
...
-- on update restrict (default): If you try to update an ID in a specific TABLE the engine will reject the operation if one USER at least links on this TABLE.
-- on delete restrict (default): If you try to delete an ID in a specific TABLE the engine will reject the operation if one USER at least links on this TABLE, can save your life.

-- In other words, the engine will reject this operations if there is a relation with other tables !